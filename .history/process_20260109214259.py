...
# グラフY軸設定
PLOT_YLIM_WORKING = None
PLOT_YLIM_INVENTORY = None
PLOT_YLIM_PRODUCTION = None
PLOT_YLIM_SHORTAGE = None
...
def visualize_demand_data(shipment_matrix, item_list, file_name):
    ...
    plt.close()
    print(f"需要データの可視化グラフを保存しました: {save_path}")

def classify_item(i):
    """
    品番を特性（High/Mid/Low）に分類する
    """
...
def read_csv(main_file_path, ship_file_path):
    ...
    if not os.path.exists(main_file_path):
        print(f"【エラー】メインファイルが見つかりません: {main_file_path}")
        sys.exit(1)
    ...
    try:
        try:
            ...
            f = open(main_file_path, 'r', encoding='utf-8')
            reader = csv.reader(f)
            header = next(reader)
        ...
        with f:
            ...
            try:
                ...
                print(f"【CSVエラー】メインファイルに必要な列名が見つかりません: {header}")
                sys.exit(1)
        ...
        print(f"【メインCSV読込エラー】: {e}")
        sys.exit(1)

    出荷数マトリクス = []
    if not ship_file_path or not os.path.exists(ship_file_path):
        print(f"【致命的エラー】需要ファイルが見つかりません: {ship_file_path}")
        sys.exit(1)
        
    print(f"詳細需要データ読み込み: {ship_file_path}")
    ship_data = {}
    try:
        try: df = pd.read_csv(ship_file_path, encoding='utf-8')
        except: df = pd.read_csv(ship_file_path, encoding='shift-jis')
    ...
    print(f"需要データマッチング完了: {len(品番リスト)}件中 {match_count}件 ヒット。")
    if missing_codes:
        print(f"【警告】未ヒット品番: {missing_codes[:5]} ...")
...
def check_jit_rule(item_idx, production_row, initial_inv_val, 期間):
    """
    【在庫のある品番は生産しないルール】をチェックする関数
    True: ルール遵守
    False: ルール違反（在庫があるのに生産している）
    """
    current_inv = initial_inv_val
    for t in range(期間):
        dem = get_demand(item_idx, t)
        
        # 生産がある場合、その日の開始時点で在庫が不足している（またはゼロ）必要がある
        # 正確には「開始在庫 < 需要」であるべき
        # つまり、「開始在庫 >= 需要」なのに生産するのはNG
...
def _op_move_to_previous_day(new_solution, initial_inventory, 期間):
    """
    【前倒し戦略】
    残業が発生している（または負荷が高い）日の生産を、
    余裕がある「前日」に移動して残業を減らす。
    在庫コストは増えるが、残業コスト削減を狙う。
    """
    ...
    for i in candidates:
        ...
        # 1. JITルールチェック（在庫過多のチェック）
        # 前倒しは在庫を増やす行為なので、JITルールに抵触しやすい。
        # ただし「残業削減」が目的のため、少し緩めるか、厳密に守るならここで弾く。
        # 今回はコードの一貫性のため check_jit_rule を守る形にします。
        ...
        # 2. 前日の負荷チェック（前日が残業にならない範囲で）
        added_load = (move_qty / 込め数リスト[i]) * サイクルタイムリスト[i]
        # 段替えが増える場合
        if original_sol[i][prev_day] == 0:
            added_load += 段替え時間
            
        if daily_loads[prev_day] + added_load > 定時 * 1.05: # 多少の許容
             # 元に戻して次へ
            new_solution[i][target_busy_day] += move_qty
            new_solution[i][prev_day] -= move_qty
            continue
...
def _op_smooth_peaks_adjacent(new_solution, initial_inventory, 期間):
    """
    【ピーク分散戦略】
    最も負荷が高い日を選び、その負荷を「前日」と「翌日」に少しずつ逃がす。
    """
    daily_loads = _compute_daily_loads(new_solution, 期間)
    peak_day = int(np.argmax(daily_loads))
    
    if daily_loads[peak_day] == 0: return new_solution

    # 分散先の候補日
    targets = []
    if peak_day > 0: targets.append(peak_day - 1)
    if peak_day < 期間 - 1: targets.append(peak_day + 1)
    
    # 候補日がどちらもピーク日より負荷が高ければ意味がないのでフィルタ
    targets = [t for t in targets if daily_loads[t] < daily_loads[peak_day]]
    if not targets: return new_solution

    # ピーク日に生産している品目からランダムに選択
    品番数 = len(initial_inventory)
    candidates = [i for i in range(品番数) if new_solution[i][peak_day] > 0]
    if not candidates: return new_solution
    
    i = random.choice(candidates)
    amount = new_solution[i][peak_day]
    if amount < 2: return new_solution # 分割できない

    # 30%〜50%を移動
    move_qty = max(1, int(amount * random.uniform(0.3, 0.5)))
    target_day = sorted(targets, key=lambda t: daily_loads[t])[0] # 最も空いている隣接日

    original_sol = [row[:] for row in new_solution]
    
    new_solution[i][peak_day] -= move_qty
    new_solution[i][target_day] += move_qty

    # 制約チェック
    valid = True
    if min(calculate_item_inventory_profile(i, new_solution[i], initial_inventory[i], 期間)) < 0:
        valid = False
    elif not check_jit_rule(i, new_solution[i], initial_inventory[i], 期間):
        valid = False
    
    if valid:
        # 新しい負荷を計算して、平準化効果があるか確認（ピークが下がっているか）
        new_loads = _compute_daily_loads(new_solution, 期間)
        if new_loads[peak_day] < daily_loads[peak_day]:
             return new_solution
...
def generate_neighbor(solution, initial_inventory, 期間=20):
    ...
    operations = [
        ...
        _op_smooth_peaks_adjacent,   # 【新規】隣接平準化
...
def plot_result(production_schedule, initial_inventory, file_name, method_name="", 期間=20):
    ...
    ax1.set_title(f'稼働時間', fontsize=18, fontweight='bold'); ax1.legend(fontsize=16)
    ...
    ax2.set_title('在庫量', fontsize=18, fontweight='bold'); ax2.legend(fontsize=16)
    ...
    for i in range(min(10, 品番数)):
        ...
        bot += short_data[i]
    ax4.set_title('出荷遅れ量', fontsize=18, fontweight='bold'); ax4.legend(fontsize=16)
